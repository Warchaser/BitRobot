...
$(function () {
    ...
    $("#sendButton1").click(function () {  //绑定发送按钮的点击事件
        var content = $("#contentText").val();  //获取输入框的内容
        if ("" == content || "\n" == content) {
            $("#messageSpan").show().html("空消息").fadeOut(2000);
            $("#contentText").val('');
...
function sendMessage1(content) {   //像服务器发送新的消息
//  var receiver = $(".mainDiv .topDiv .rightDiv #onlineDiv ul li div:contains('聊天对象') label:first").text().trim();
...
//      data: "content=" + content + "sender=" + sender,
      ...
      success: function (data) {
          if (1 == data.sendResult) {
              $("#contentText").val("");
                  addNewMsgLift(data.sendTime, data.content);
              $("#messageSpan").html("已发送").fadeOut(2000);
          }
          else {
              $("#messageSpan").html("发送失败").fadeOut(2000);
...
function addNewMsgRight(sendTime, content) {
    ...
    var newDiv = "<div id='rightdiv'></div>";
    
    var newDiv1 = "<div ><h6>" + sendTime + "</h6>&nbsp;";
...
function addNewMsgLift(sendTime, content) {
	var messageDiv = $("#messageDiv");
    var newDiv2 = "<div id='liftdiv'></div>";
    
    var newDiv3 = "<div ><h6>" + sendTime + "</h6>&nbsp;";
 
    newDiv3 += "<br />";

    newDiv3 += content + "</div>";

    messageDiv.append(newDiv2);
